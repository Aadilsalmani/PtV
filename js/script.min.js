...
window.addEventListener("DOMContentLoaded", () => {
  ...
  window.sharePlace = async function(url, name) {
    try {
      const shareText = `${name} — Found on The P2V\nExplore it: ${url}\nhttps://thep2v.com`;

      if (navigator.share) {
        await navigator.share({ title: `${name} — The P2V`, text: shareText, url });
        try { gtag && gtag('event', 'place_shared', { event_category: 'engagement', event_label: name }); } catch(e){}
        return;
      }
      if (navigator.clipboard && navigator.clipboard.writeText) {
        await navigator.clipboard.writeText(shareText);
        alert("Link copied to clipboard — paste in WhatsApp/Telegram to share!");
        try { gtag && gtag('event', 'place_link_copied', { event_category: 'engagement', event_label: name }); } catch(e){}
        return;
      }
      window.prompt("Copy this link to share:", shareText);
    } catch (err) {
