...
let allPlaces = [], markers = [], userLat, userLon, userMarker = null;
let userStatus = JSON.parse(localStorage.getItem("userStatus")) || {};
let watchId = null;

// 🧭 Distance function
function getDistance(e, t, a, o) {
  const R = 6371, dLat = (a - e) * Math.PI / 180, dLon = (o - t) * Math.PI / 180;
  ...
  watchId = navigator.geolocation.watchPosition(
    pos => {
      ...
      if (!userMarker) {
        ...
        userMarker.setLatLng([userLat, userLon]).bindPopup("📍 You are here");
      }

      map.flyTo([userLat, userLon], 12, { animate: true, duration: 1 });
      applyFilters();
    },
    err => {
...
function applyFilters() {
  try {
    ...
    let list = allPlaces.filter(item => {
      ...
      userLat && userLon ? item.Distance = getDistance(userLat, userLon, item.Latitude, item.Longitude) : item.Distance = null;
    ...
    list.slice(0, 200).forEach(item => {
      ...
      marker.bindPopup(`
        ...
        <button class="dir-btn" onclick="openDirections(${item.Latitude},${item.Longitude},'${(item.Name || "").replace(/'/g, "\\'")}')">🧭 Get Directions</button>
      `);
      markers.push(marker);
...
// 🌍 Fetch data safely
fetch("https://script.google.com/macros/s/AKfycbw6ZXWi9U1pzumYL-o7aUSPoJSGekdCrL7mEkLaFaVQU803q5LqsW5_UXVZH8PR11jV/exec")
  .then(res => res.json())
  .then(data => { allPlaces = data; restoreFilters(); locateUser(); })
  .catch(err => {
    console.error("Data load error:", err);
    locateUser(); // still load map if fetch fails
  });

// ✅ SW + Feedback handlers (unchanged)
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  submitBtn?.addEventListener("click", async () => {
    ...
    if (!text) return alert("Please write something before sending feedback.");

    submitBtn.disabled = true;
    submitBtn.textContent = "⏳ Sending...";
    submitBtn.style.opacity = "0.6";

    const data = { message: text, timestamp: new Date().toLocaleString(), userLocation: { lat: userLat || null, lon: userLon || null } };
    try {
      ...
      feedbackText.value = ""; feedbackModal.classList.remove("show");
    } catch (err) {
      console.error("Feedback submission failed:", err);
      alert("⚠️ Unable to send feedback. Try again later.");
    } finally {
      setTimeout(() => { submitBtn.disabled = false; submitBtn.textContent = "Send"; submitBtn.style.opacity = "1"; }, 2000);
    }
  });
});

// 🕒 Guaranteed Splash Removal (failsafe)
window.addEventListener("load", () => {
  ...
  }, 2000); // hide after 2s no matter what
});
