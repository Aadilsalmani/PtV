  ...
  function applyFilters() {
    try {
      ...
      list.slice(0, 200).forEach(item => {
        ...
        // ✅ Keep popup open until manually closed
        marker.bindPopup(
          `
          <div style="min-width:160px">
            ...
            <button class="dir-btn" onclick="openDirections(${item.Latitude},${item.Longitude},'${(item.Name || "").replace(/'/g, "\\'")}')">
              🧭 Get Directions
            </button>
          </div>
          `,
          {
            ...
            closeOnClick: false,   // ❌ don’t close when map clicked
        ...
        // ✅ Attach marker open behavior cleanly
        marker.on("click", () => {
          marker.openPopup();
        });
        markers.push(marker);
        ...
        // Smooth popup open after map pans
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (infoBtn && aboutModal) {
    ...
    console.warn("ℹ️ Info button or modal not found yet — check HTML placement");
  }
});
