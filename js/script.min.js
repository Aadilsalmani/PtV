  ...
  (function checkDeepLinkFromUrl() {
    try {
      ...
      if (lat && lon) {
        ...
        window.__P2V_skipAutoFollow = true;
  ...
  function locateUser() {
    ...
    watchId = navigator.geolocation.watchPosition(
      pos => {
        ...
        if (autoFollow && !window.__P2V_skipAutoFollow && !deepLinkHandled) {
  ...
  function applyFilters() {
    ...
    list.slice(0, 200).forEach(function(item) {
      try {
        ...
        // Build short permalink
        const placeSlug = encodeURIComponent((item.Name || '').trim().replace(/\s+/g, '-').toLowerCase());
        const placeUrl = location.origin + '/?place=' + placeSlug + '&lat=' + item.Latitude + '&lon=' + item.Longitude;
    ...
    if (pendingDeepLink) {
      try {
        ...
        if (!targetMarker && dl.placeSlug) {
          ...
          for (let i = 0; i < markers.length; i++) {
            ...
            const found = allPlaces.find(p => {
              ...
              const pslug = (p.Name || '').toString().trim().replace(/[^a-z0-9\-]/gi, '').replace(/\s+/g, '-').toLowerCase();
        ...
        if (!targetMarker) {
          ...
          setTimeout(() => { window.__P2V_skipAutoFollow = false; }, 800);
        } else {
          ...
          setTimeout(() => {
            try {
              ...
              setTimeout(() => { window.__P2V_skipAutoFollow = false; }, 600);
            }
          }, 250);
        }
      } catch (err) {
        ...
        setTimeout(() => {
          ...
          window.__P2V_skipAutoFollow = false;
        }, 800);
      }
    }
