    ...
    if (pendingDeepLink) {
      try {
        ...
        if (!targetMarker) {
          ...
          // Prefer map.openPopup() on the marker's popup instance
          const popupInstance = (targetMarker.getPopup && targetMarker.getPopup()) || null;
          if (popupInstance) {
            map.openPopup(popupInstance, latlng);
          } else {
            // fallback
            try { targetMarker.openPopup(); } catch(e){ console.warn("marker.openPopup fallback failed", e); }
          }
